...
@Component({
  ...
  styles: [`
    ...
    .btn-logout { background: rgba(255,255,255,0.1); border: none; color: white;
                  cursor: pointer; padding: 0.4rem 0.6rem; border-radius: 6px; font-size: 1rem; flex-shrink: 0; }
    ...
    .btn-toggle { background: none; border: none; font-size: 1.2rem; cursor: pointer; color: #555; padding: 0.25rem 0.5rem; }
...
export class LayoutComponent {
  ...
  // Computed properties pour éviter l'erreur "Object is of type unknown"
  userEmail = computed(() => this.authService.currentUser()?.email ?? '');
  userRole  = computed(() => this.authService.currentUser()?.role ?? '');
  isAdmin   = computed(() => this.authService.hasRole(['SUPER_ADMIN']));
}
